...
## In-Place Update (Image and Resource Changes)

For sandboxes that only need an **image** or **resource** change (without modifying other pod spec fields), the controller supports **in-place updates**. This is more lightweight than the Recreate upgrade and does not destroy the running pod.

### How It Works

1. When `spec.template` changes, the controller computes two hashes:
   - **Full template hash** (`updateRevision`): Includes all fields.
   - **Immutable part hash** (`sandbox.agents.kruise.io/hash-immutable-part`): Excludes metadata, images, and resources.

2. If only the full hash changes (but the immutable hash remains the same), the change is limited to image/resources, and an **in-place update** is triggered.

3. If the immutable hash also changes, the change involves other fields (e.g., volumes, command), and a Recreate upgrade is required instead.

### Supported In-Place Update Fields

| Field | In-Place Update Supported |
|---|---|
| Container `image` | Yes |
| Container `resources` (requests/limits) | Yes (requires Kubernetes 1.27+ for resize) |
| Container `command`, `args` | No (requires Recreate) |
| Volumes, volume mounts | No (requires Recreate) |
| Pod labels, annotations | No (requires Recreate) |
| Other pod spec fields | No (requires Recreate) |

### In-Place Update via SandboxClaim

When claiming a sandbox from a pre-warmed pool, you can perform an in-place update as part of the claim process:
...
This claims a sandbox from the pool and immediately updates its image and resources in-place without recreating the pod.
...
### SandboxUpgradePolicy

| Field | Type | Description |
|---|---|---|
| `type` | `string` | Upgrade policy type. Supported values: `Recreate`. When empty (default), upgrading is disabled. |

### SandboxLifecycle

| Field | Type | Description |
|---|---|---|
| `preUpgrade` | `UpgradeAction` | Action to execute before the upgrade (e.g., backup). |
| `postUpgrade` | `UpgradeAction` | Action to execute after the upgrade (e.g., restore). |

### UpgradeAction

| Field | Type | Default | Description |
|---|---|---|---|
| `exec` | `ExecAction` | - | Command to execute inside the sandbox via agent-runtime. |
| `timeoutSeconds` | `int32` | 60 | Timeout for the action execution in seconds. |

### SandboxSetUpdateStrategy

| Field | Type | Default | Description |
|---|---|---|---|
| `maxUnavailable` | `IntOrString` | `"20%"` | Maximum number or percentage of sandboxes that can be unavailable during the rolling update. |

### SandboxUpdateOpsSpec

| Field | Type | Description |
|---|---|---|
| `selector` | `LabelSelector` | Selects the target sandboxes to update by label. |
| `updateStrategy.maxUnavailable` | `IntOrString` | Maximum number of sandboxes that can be upgrading simultaneously. |
| `patch` | `RawExtension` | Strategic Merge Patch to apply to each sandbox's PodTemplateSpec. |
| `lifecycle` | `SandboxLifecycle` | Pre/post upgrade hooks to set on each sandbox. |
| `paused` | `bool` | Whether the update operation is paused. |
